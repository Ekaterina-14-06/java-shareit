...
class ItemControllerTest {
    ...
    private ReviewDto createReviewDto() {
        return new ReviewDto(1L, "Great", "user1", LocalDateTime.now());
    }
    ...
    void findAllItemsTest() throws Exception {
        ...
        mockMvc.perform(get("/items")
                ...
                .andExpect(content().json("[{\"id\": 1,\"name\": \"item1\"," +
                        ...
                        " \"lastBooking\": null, \"nextBooking\": null, \"reviews\": []}]"));
    ...
    @Test
    void createReviewForItemTest() throws Exception {
        ReviewDto reviewDto = createReviewDto();
        Item item = createItem();
        when(itemService.saveReview(1, 1, reviewDto))
                .thenReturn(reviewDto);
        mockMvc.perform(post("/items/1/review")
                ...
                .andExpect(content().json("{\"id\": 1,\"text\": \"Great\"," +
                        " \"authorName\": \"user1\"}"));
        verify(itemService, times(1))
                .saveReview(item.getOwner().getId(), item.getId(), reviewDto);
    }
